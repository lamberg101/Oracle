explain plan for
SELECT * FROM ( SELECT row_number() over ( order by txn_dt_time ) as no, 
...
AND a.TRN_REF_NO NOT IN (SELECT TRN_REF_NO FROM actb_daily_log WHERE trn_code in ('445','446') and event='REVR')
AND a.cust_gl='A'
